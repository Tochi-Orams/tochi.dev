...
const MobileNav = () => {
  ...
  const [activeTitle, setActiveTitle] = useState("translate-y-0");
  ...
  useEffect(() => {
    if (currentSection === "about") {
      setActiveTitle("-translate-y-[3rem]");
    } else if (currentSection === "projects") {
      setActiveTitle("-translate-y-[6rem]");
    } else if (currentSection === "contact") {
      setActiveTitle("-translate-y-[9rem]");
    } else {
      setActiveTitle("translate-y-0");
    }
  }, [currentSection]);

  return (
    <div
      className={`fixed z-30 xm:top-[21.5px] sm:top-7 top-6 lg:right-[100px] sm:right-10 right-3 sm:left-112 left-6 duration-1000 delay-[2s] lg:!hidden block ${
        pageLoaded
    ...
    >
      <div className="flex justify-between w-full items-center">
        <div
          className={`mt-[0.5rem] pr-6 h-[3rem] overflow-hidden border-b-2 border-white w-fit duration-300 ${
            currentSection === "home" || hover ? "opacity-0" : "opacity-100"
          }`}
        >
          <div
            className={`flex flex-col items-start ${activeTitle} duration-300`}
          >
            {navPages.map((page, i) => (
              <p key={i} id={page.id} className="h-[3rem] h2Size text-nowrap">
                {page.text.split(" ")[0]}
              </p>
            ))}
          </div>
